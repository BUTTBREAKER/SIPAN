...
class Caja extends BaseModel
{
     ...
     * Optimización Bolt: Implementa cacheo a nivel de request para evitar consultas redundantes (O(N) -> O(1)).
     */
    public function getActiva($id_sucursal)
    {
        ...
        $caja = $this->db->fetchOne($sql, [$id_sucursal]);

        self::$activeCajaCache[$id_sucursal] = $caja;
        return $caja;
    }
     ...
     * Optimización Bolt: Consolida la obtención del monto de apertura y la agregación de movimientos
     * en una sola consulta SQL utilizando LEFT JOIN y SUM(CASE), reduciendo round-trips de 2 a 1.
     */
