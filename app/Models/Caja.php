...
class Caja extends BaseModel
{
    ...
    /**
     * Request-level cache for active cash box
     * @var array
     */
    protected static $activeCajaCache = [];

    /**
     * Obtiene la caja activa para una sucursal
     * Bolt Optimization: Uses request-level static cache to avoid redundant DB queries.
     */
     ...
     * Bolt Optimization: Invalidates active caja cache on successful open.
     */
     ...
     * Bolt Optimization: Consolidated two database round-trips into one using a LEFT JOIN
     * and conditional aggregation. Includes a safety check to return 0.0 if no result is found.
     */
    public function getResumen($id_caja)
    {
        $sql = "SELECT c.monto_apertura,
     ...
     * Bolt Optimization: Invalidates active caja cache on successful close.
     */
