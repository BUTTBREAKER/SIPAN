...
class ChatMensaje
{
    ...
    {
        $sql = "SELECT 
                    c.id, c.tipo, c.nombre AS grupo_nombre, c.id_sucursal,
                    m.mensaje AS ultimo_mensaje, m.created_at AS ultimo_mensaje_fecha,
                ...
                -- Bolt Optimization: Replaced correlated scalar subquery for unread count with a JOIN to a derived table.
                -- Subquery is filtered by userId to keep intermediate results small (O(N+M)).
                LEFT JOIN (
                    SELECT cm.id_conversacion, COUNT(*) as count
                    FROM chat_mensajes cm
                    JOIN chat_participantes cp_sub ON cm.id_conversacion = cp_sub.id_conversacion
                ...
                -- Bolt Optimization: Replaced correlated scalar subquery for latest message ID with a JOIN to a derived table.
                -- Uses MAX(id) for efficiency (O(N+M)).
                LEFT JOIN (
                    SELECT m2.id_conversacion, MAX(m2.id) as last_id
                    FROM chat_mensajes m2
                    JOIN chat_participantes cp_sub ON m2.id_conversacion = cp_sub.id_conversacion
                    WHERE cp_sub.id_usuario = ?
    ...
    {
        // Bolt Optimization: Replaced nested scalar subquery (O(N*M)) with a single JOIN (O(N+M)).
        $sql = "SELECT COUNT(*) AS total
                FROM chat_participantes cp
                JOIN chat_mensajes cm ON cp.id_conversacion = cm.id_conversacion
                WHERE cp.id_usuario = ?
                  AND cm.id_usuario != ?
