...
class ChatMensaje
{
    ...
    public function getConversaciones($userId)
    {
        $sql = "SELECT 
                ...
                -- Bolt Optimization: Last message via pre-filtered derived table (O(N+M) instead of O(N*M))
                LEFT JOIN (
                    SELECT m2.id_conversacion, MAX(m2.id) as last_id
                    FROM chat_mensajes m2
                    INNER JOIN chat_participantes cp_msg ON cp_msg.id_conversacion = m2.id_conversacion
                ...
                -- Bolt Optimization: Unread counts via pre-filtered derived table (O(N+M) instead of O(N*M))
                LEFT JOIN (
    ...
    {
        // Bolt Optimization: Use direct JOIN with aggregation (O(N+M)) instead of correlated subquery (O(N*M))
