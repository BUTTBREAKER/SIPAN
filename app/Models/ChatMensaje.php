...
class ChatMensaje
{
     ...
     * Optimización Bolt: Uso de tablas derivadas para evitar subconsultas correlacionadas y dependent subqueries.
     */
    public function getConversaciones($userId)
    {
        $sql = "SELECT 
                    ...
                    COALESCE(unreads.cnt, 0) AS no_leidos,
                ...
                -- Último mensaje optimizado: Evita subconsultas dependientes por fila
                LEFT JOIN (
                    SELECT m1.id_conversacion, MAX(m1.id) as last_id
                    FROM chat_mensajes m1
                    INNER JOIN chat_participantes cp_msg ON cp_msg.id_conversacion = m1.id_conversacion
                    WHERE cp_msg.id_usuario = ?
                    GROUP BY m1.id_conversacion
                ...
                -- Conteo no leídos optimizado: Agregación en un solo paso
                LEFT JOIN (
                    SELECT cm2.id_conversacion, COUNT(*) as cnt
                    FROM chat_mensajes cm2
                    INNER JOIN chat_participantes cp2 ON cp2.id_conversacion = cm2.id_conversacion
                    WHERE cp2.id_usuario = ?
                      AND cm2.created_at > COALESCE(cp2.ultimo_leido, '1970-01-01')
                      AND cm2.id_usuario != ?
                    GROUP BY cm2.id_conversacion
                ) unreads ON unreads.id_conversacion = c.id
     ...
     * Optimización Bolt: Reemplazo de subconsulta correlacionada por un solo JOIN eficiente.
     */
    public function contarNoLeidos($userId)
    {
        $sql = "SELECT COUNT(*) AS total
                FROM chat_mensajes cm
                INNER JOIN chat_participantes cp ON cp.id_conversacion = cm.id_conversacion
                WHERE cp.id_usuario = ?
