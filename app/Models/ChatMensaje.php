...
class ChatMensaje
{
     ...
     * Optimizado: Se eliminan subconsultas correlacionadas pesadas en SELECT y JOIN,
     * reemplazándolas con un JOIN a tablas derivadas que filtran por usuario.
     */
    public function getConversaciones($userId)
    {
        $sql = "SELECT 
                ...
                -- Último mensaje (tabla derivada optimizada con MAX para evitar subconsulta correlacionada)
                LEFT JOIN (
                    SELECT m2.id_conversacion, MAX(m2.id) AS max_id
                    FROM chat_mensajes m2
                    INNER JOIN chat_participantes cp_m ON cp_m.id_conversacion = m2.id_conversacion
                ...
                -- Conteo de no leídos por conversación para este participante
                LEFT JOIN (
     ...
     * Optimizado: Se elimina la subconsulta correlacionada pesada y sub-queries anidados,
     * transformando todo a un JOIN directo de alta eficiencia indexada.
     */
