...
class ChatMensaje
{
     ...
     * Optimización Bolt: Se refactorizaron las subconsultas correlacionadas en Joins
     * con tablas derivadas (O(N+M)) para mejorar drásticamente el rendimiento en
    ...
    public function getConversaciones($userId)
    {
        $sql = "SELECT 
                ...
                -- Optimización Bolt: Join con tabla derivada para último mensaje (evita subquery por fila)
                LEFT JOIN (
                ...
                -- Optimización Bolt: Join con tabla derivada para no leídos (evita subquery por fila)
                LEFT JOIN (
                    SELECT cm.id_conversacion, COUNT(*) as count
                    FROM chat_mensajes cm
                    INNER JOIN chat_participantes cp_inner ON cp_inner.id_conversacion = cm.id_conversacion
                    WHERE cp_inner.id_usuario = ?
                      AND cm.created_at > COALESCE(cp_inner.ultimo_leido, '1970-01-01')
     ...
     * Optimización Bolt: Se eliminaron las subconsultas correlacionadas (O(N*M))
     * a favor de un JOIN directo (O(N+M)), reduciendo la carga en la DB.
     */
