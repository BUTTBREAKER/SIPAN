...
class ChatMensaje
{
    ...
    {
        // Bolt Optimization: Refactored to eliminate correlated subqueries (O(N*M) -> O(N+M)).
        // Using derived table joins for latest messages and unread counts.
        $sql = "SELECT 
                    c.id,
                    c.tipo,
                ...
                -- Último mensaje (Optimizado: join con tabla derivada de MAX(id))
                LEFT JOIN (
                    SELECT id_conversacion, MAX(id) as max_id
                    FROM chat_mensajes
                    GROUP BY id_conversacion
                ) m_max ON m_max.id_conversacion = c.id
                LEFT JOIN chat_mensajes m ON m.id = m_max.max_id
                LEFT JOIN usuarios m_user ON m_user.id = m.id_usuario
                -- Conteo no leídos (Optimizado: join con agregación por usuario)
                LEFT JOIN (
                    SELECT cm.id_conversacion, cp_inner.id_usuario, COUNT(*) as total
                    FROM chat_mensajes cm
                    INNER JOIN chat_participantes cp_inner ON cm.id_conversacion = cp_inner.id_conversacion
                    WHERE cm.created_at > COALESCE(cp_inner.ultimo_leido, '1970-01-01')
                      AND cm.id_usuario != cp_inner.id_usuario
                    GROUP BY cm.id_conversacion, cp_inner.id_usuario
                ) unread ON unread.id_conversacion = c.id AND unread.id_usuario = cp.id_usuario
    ...
    {
        // Bolt Optimization: Refactored to use a single JOIN for unread counts (O(N*M) -> O(N+M)).
        // This avoids nested subqueries and significantly reduces DB load during polling.
