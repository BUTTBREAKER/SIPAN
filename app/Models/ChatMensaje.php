...
class ChatMensaje
{
     ...
     * Bolt Optimization: Replaced correlated scalar subqueries with JOINs to filtered derived tables
     * to reduce complexity from O(N*M) to O(N+M).
     */
    public function getConversaciones($userId)
    {
        $sql = "SELECT 
                ...
                -- Bolt Optimization: Join with a derived table for unread counts (O(N+M))
                LEFT JOIN (
                    ...
                    WHERE cp_unread.id_usuario = ?
                      ...
                      AND cm2.id_usuario != cp_unread.id_usuario
                    GROUP BY cm2.id_conversacion
                ) unread ON unread.id_conversacion = c.id
                -- Bolt Optimization: Join with a derived table for the latest message ID (O(N+M))
                LEFT JOIN (
                    SELECT cm3.id_conversacion, MAX(cm3.id) as last_id
                    FROM chat_mensajes cm3
                    INNER JOIN chat_participantes cp_msg ON cm3.id_conversacion = cp_msg.id_conversacion
                    WHERE cp_msg.id_usuario = ?
                    GROUP BY cm3.id_conversacion
                ) latest ON latest.id_conversacion = c.id
                LEFT JOIN chat_mensajes m ON m.id = latest.last_id
     ...
     * Bolt Optimization: Replaced O(N*M) correlated subquery with a direct JOIN (O(N+M)).
     */
    public function contarNoLeidos($userId)
    {
        $sql = "SELECT COUNT(*) AS total
                ...
                WHERE cp.id_usuario = ?
                  ...
                  AND cm.id_usuario != cp.id_usuario";
