...
class ChatMensaje
{
    ...
    {
        // Bolt Optimization: Refactored correlated subqueries into derived tables for O(N+M) complexity.
        // Also added context-aware filters inside subqueries to minimize data processing and enable index usage.
        $sql = "SELECT 
                    c.id,
                    c.tipo,
                    ...
                    COALESCE(nl.count, 0) AS no_leidos,
                ...
                -- Optimization: Latest message via MAX(id) join restricted to user's conversations
                LEFT JOIN (
                    SELECT cm1.id_conversacion, MAX(cm1.id) as max_id
                    FROM chat_mensajes cm1
                    INNER JOIN chat_participantes cp_filter ON cp_filter.id_conversacion = cm1.id_conversacion
                    WHERE cp_filter.id_usuario = ?
                    GROUP BY cm1.id_conversacion
                ) latest_m ON latest_m.id_conversacion = c.id
                LEFT JOIN chat_mensajes m ON m.id = latest_m.max_id
                LEFT JOIN usuarios m_user ON m_user.id = m.id_usuario
                -- Optimization: Unread counts via pre-aggregated join restricted to user's conversations
                LEFT JOIN (
                    SELECT cm2.id_conversacion, COUNT(*) as count
                    FROM chat_mensajes cm2
                    INNER JOIN chat_participantes cp_filter ON cp_filter.id_conversacion = cm2.id_conversacion
                    WHERE cp_filter.id_usuario = ?
                      AND cm2.created_at > COALESCE(cp_filter.ultimo_leido, '1970-01-01')
                      AND cm2.id_usuario != ?
                    GROUP BY cm2.id_conversacion
                ) nl ON nl.id_conversacion = c.id
                -- Other participant (direct)
    ...
    {
        // Bolt Optimization: Replaced scalar subquery with a single JOIN.
        // Complexity reduced from O(N*M) to O(N+M) where N=participantes, M=mensajes.
        $sql = "SELECT COUNT(*) AS total
                FROM chat_participantes cp
                INNER JOIN chat_mensajes cm ON cp.id_conversacion = cm.id_conversacion
                WHERE cp.id_usuario = ?
                  AND cm.id_usuario != ?
