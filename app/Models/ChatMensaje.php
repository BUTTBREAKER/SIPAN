...
class ChatMensaje
{
    ...
    {
        // Bolt Optimization: Refactored scalar and correlated subqueries into derived table joins.
        // This reduces complexity from O(N*M) to O(N+M) and avoids redundant table scans
        // for unread counts and latest message retrieval.
        $sql = "SELECT 
                    c.id,
                    c.tipo,
                ...
                -- Bolt: Latest message via derived table (MAX(id) for better performance than correlated LIMIT 1)
                LEFT JOIN (
                    SELECT id_conversacion, MAX(id) as last_id
                    FROM chat_mensajes
                ...
                -- Bolt: Unread counts via derived table with context-aware filters
                LEFT JOIN (
    ...
    {
        // Bolt Optimization: Refactored nested subqueries into a single efficient INNER JOIN.
