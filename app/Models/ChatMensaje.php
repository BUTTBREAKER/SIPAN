...
class ChatMensaje
{
     ...
     * Optimización Bolt: Reemplazadas subconsultas correlacionadas por JOINs a tablas derivadas
     * para mantener complejidad O(N+M) y evitar escaneos repetitivos.
     */
    public function getConversaciones($userId)
    {
        $sql = "SELECT 
                    ...
                    -- Conteo no leídos (desde tabla derivada)
                ...
                -- Conteo no leídos pre-agregado por conversación
                LEFT JOIN (
                    SELECT cm2.id_conversacion, COUNT(*) as total
                    FROM chat_mensajes cm2
                    INNER JOIN chat_participantes cp_inner ON cm2.id_conversacion = cp_inner.id_conversacion
                    WHERE cp_inner.id_usuario = ?
                      AND cm2.id_usuario != ?
                      AND cm2.created_at > COALESCE(cp_inner.ultimo_leido, '1970-01-01')
                    GROUP BY cm2.id_conversacion
                ) unr ON unr.id_conversacion = c.id
                -- Último mensaje (vía JOIN a MAX id para evitar subconsulta correlacionada ORDER BY LIMIT 1)
                LEFT JOIN (
                    SELECT id_conversacion, MAX(id) as last_msg_id
                    FROM chat_mensajes
                    GROUP BY id_conversacion
                ) last_msg ON last_msg.id_conversacion = c.id
                LEFT JOIN chat_mensajes m ON m.id = last_msg.last_msg_id
     ...
     * Optimización Bolt: Reemplazada subconsulta correlacionada por un JOIN directo
     * para reducir complejidad de O(N*M) a O(N+M).
     */
