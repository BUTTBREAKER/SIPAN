...
class ChatMensaje
{
     ...
     * Optimización Bolt: Reemplazadas las subconsultas correlacionadas por JOINs a tablas derivadas
     * con filtro de usuario interno para mantener complejidad O(N+M) en el polling.
     */
    public function getConversaciones($userId)
    {
        $sql = "SELECT 
                ...
                -- Último mensaje (derivada m_last filtrada por usuario)
                LEFT JOIN (
                    SELECT cm_max.id_conversacion, MAX(cm_max.id) AS max_id
                    FROM chat_participantes cp_m
                    INNER JOIN chat_mensajes cm_max ON cm_max.id_conversacion = cp_m.id_conversacion
                ...
                -- Conteo no leídos (derivada unread filtrada por usuario)
                LEFT JOIN (
                    SELECT cm.id_conversacion, COUNT(*) AS no_leidos
                    FROM chat_participantes cp_sub
                    INNER JOIN chat_mensajes cm
                        ON cm.id_conversacion = cp_sub.id_conversacion
                       AND cm.created_at > COALESCE(cp_sub.ultimo_leido, '1970-01-01')
                       AND cm.id_usuario != cp_sub.id_usuario
     ...
     * Optimización Bolt: Reemplazadas subconsultas correlacionadas por INNER JOIN directo (O(N+M)).
     */
    public function contarNoLeidos($userId)
    {
        $sql = "SELECT COUNT(cm.id) AS total
                FROM chat_participantes cp
                INNER JOIN chat_mensajes cm
                    ON cm.id_conversacion = cp.id_conversacion
