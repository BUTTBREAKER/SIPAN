...
class ChatMensaje
{
    ...
    {
        // Bolt Optimization: Replaced correlated scalar subqueries (O(N*M)) with JOINs to derived tables.
        // This ensures complexity stays at O(N+M) even as the message volume grows.
        // Derived tables are filtered by the user's conversations to maintain performance.
        $sql = "SELECT 
                    c.id,
                    c.tipo,
                ...
                -- Último mensaje (vía derived table para evitar correlated subquery)
                LEFT JOIN (
                    SELECT cm1.id_conversacion, MAX(cm1.id) as max_id
                    FROM chat_mensajes cm1
                    JOIN chat_participantes cp1 ON cm1.id_conversacion = cp1.id_conversacion
                    WHERE cp1.id_usuario = ?
                    GROUP BY cm1.id_conversacion
                ) last_msg_ids ON last_msg_ids.id_conversacion = c.id
                LEFT JOIN chat_mensajes m ON m.id = last_msg_ids.max_id
                LEFT JOIN usuarios m_user ON m_user.id = m.id_usuario
                -- Conteo no leídos (vía derived table para evitar correlated subquery)
                LEFT JOIN (
                    SELECT cm2.id_conversacion, COUNT(*) as unread_count
                    FROM chat_mensajes cm2
                    JOIN chat_participantes cp2 ON cm2.id_conversacion = cp2.id_conversacion
                    WHERE cp2.id_usuario = ? AND cm2.id_usuario != ?
                ...
                LEFT JOIN chat_participantes cp3 ON cp3.id_conversacion = c.id
                    AND cp3.id_usuario != ? AND c.tipo = 'directa'
                LEFT JOIN usuarios other_user ON other_user.id = cp3.id_usuario
     ...
     * Bolt Optimization: Replaced correlated scalar subquery (O(N*M)) with a flat JOIN (O(N+M)).
     */
    public function contarNoLeidos($userId)
    {
        $sql = "SELECT COUNT(m.id) AS total
                FROM chat_participantes cp
                JOIN chat_mensajes m ON cp.id_conversacion = m.id_conversacion
                WHERE cp.id_usuario = ?
                  AND m.id_usuario != ?
                  AND m.created_at > COALESCE(cp.ultimo_leido, '1970-01-01')";
