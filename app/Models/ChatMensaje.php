...
class ChatMensaje
{
    ...
    public function getConversaciones($userId)
    {
        $sql = "SELECT 
                ...
                -- Optimized Bolt: Join with unread counts derived table (O(N+M))
                LEFT JOIN (
                    SELECT cm2.id_conversacion, COUNT(*) as total
                    FROM chat_mensajes cm2
                    INNER JOIN chat_participantes cp_sub ON cm2.id_conversacion = cp_sub.id_conversacion
                    WHERE cp_sub.id_usuario = ?
                      AND cm2.created_at > COALESCE(cp_sub.ultimo_leido, '1970-01-01')
                      AND cm2.id_usuario != ?
                    GROUP BY cm2.id_conversacion
                ) AS unread ON unread.id_conversacion = c.id
                -- Optimized Bolt: Join with latest message ID derived table (O(N+M))
                LEFT JOIN (
                    SELECT cm3.id_conversacion, MAX(cm3.id) as last_id
                    FROM chat_mensajes cm3
                    INNER JOIN chat_participantes cp_sub2 ON cm3.id_conversacion = cp_sub2.id_conversacion
                    WHERE cp_sub2.id_usuario = ?
                    GROUP BY cm3.id_conversacion
                ) AS last_msg ON last_msg.id_conversacion = c.id
    ...
    {
        // Optimized Bolt: Direct join between participants and messages (O(N+M))
