...
class ChatMensaje
{
     ...
     * Optimización Bolt: Se reemplazaron subconsultas correlacionadas (O(N*M)) por JOINs con tablas derivadas (O(N+M)).
     */
    public function getConversaciones($userId)
    {
        $sql = "SELECT 
                ...
                -- Bolt: Último mensaje mediante JOIN con tabla derivada de MAX(id)
                LEFT JOIN (
                    SELECT m_max.id_conversacion, MAX(m_max.id) as max_id
                    FROM chat_mensajes m_max
                    GROUP BY m_max.id_conversacion
                ...
                -- Bolt: Conteo de no leídos mediante JOIN con tabla derivada filtrada por usuario
                LEFT JOIN (
                    SELECT cm_u.id_conversacion, COUNT(*) as total
                    FROM chat_mensajes cm_u
                    INNER JOIN chat_participantes cp_u ON cm_u.id_conversacion = cp_u.id_conversacion
                    WHERE cp_u.id_usuario = ?
                      AND cm_u.created_at > COALESCE(cp_u.ultimo_leido, '1970-01-01')
                      AND cm_u.id_usuario != ?
                    GROUP BY cm_u.id_conversacion
     ...
     * Optimización Bolt: Refactorizado de subconsultas anidadas a un solo JOIN con filtrado directo (O(N+M)).
     */
    public function contarNoLeidos($userId)
    {
        $sql = "SELECT COUNT(m.id) as total
                FROM chat_participantes cp
                INNER JOIN chat_mensajes m ON m.id_conversacion = cp.id_conversacion
                WHERE cp.id_usuario = ?
