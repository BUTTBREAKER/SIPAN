...
class ChatMensaje
{
    ...
    public function getConversaciones($userId)
    {
        ...
        // This reduces complexity from O(N*M) to O(N+M) and avoids redundant scans.
        // Post-Review Fix: Added user filters inside derived tables to prevent table-wide scans.
        $sql = "SELECT 
                    c.id,
                    c.tipo,
                    ...
                    -- Conteo no leídos
                    COALESCE(un.no_leidos, 0) AS no_leidos,
                ...
                -- Último mensaje (JOIN con tabla derivada filtrada por las conversaciones del usuario)
                LEFT JOIN (
                    SELECT m2.id_conversacion, MAX(m2.id) as last_id
                    FROM chat_mensajes m2
                    INNER JOIN chat_participantes cp_sub ON m2.id_conversacion = cp_sub.id_conversacion
                ...
                -- Conteo no leídos (JOIN con tabla derivada filtrada por las conversaciones del usuario)
                LEFT JOIN (
    ...
    {
        // Bolt Optimization: Refactored nested subqueries into a single INNER JOIN.
        // This is significantly more efficient as it performs a single scan of the messages table.
