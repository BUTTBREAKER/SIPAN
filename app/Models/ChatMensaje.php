...
class ChatMensaje
{
     ...
     * Optimización Bolt: Reemplazadas subconsultas correlacionadas por tablas derivadas (JOINs).
     * Se filtran los mensajes por participación del usuario para evitar escaneos globales.
     */
    public function getConversaciones($userId)
    {
        $sql = "SELECT 
                ...
                -- Último mensaje (tabla derivada para O(N+M) en lugar de O(N*M))
                LEFT JOIN (
                    SELECT m2.id_conversacion, MAX(m2.id) as last_id
                    FROM chat_mensajes m2
                    INNER JOIN chat_participantes cp_filter ON m2.id_conversacion = cp_filter.id_conversacion
                ...
                -- Conteo no leídos (tabla derivada)
                LEFT JOIN (
     ...
     * Optimización Bolt: Reemplazada subconsulta correlacionada por un JOIN con filtrado por usuario.
     */
