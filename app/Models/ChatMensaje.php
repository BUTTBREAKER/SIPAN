...
class ChatMensaje
{
    ...
    public function getConversaciones($userId)
    {
        ...
        // This reduces execution complexity from O(N*M) to O(N+M), significantly improving
        // performance as message and conversation history grows.
        $sql = "SELECT 
                    c.id,
                    c.tipo,
                    ...
                    COALESCE(unr.count, 0) AS no_leidos,
                ...
                -- Optimization: Use MAX(id) in a derived table to find latest message efficiently
                LEFT JOIN (
                ...
                -- Optimization: Join with unread counts derived table
                LEFT JOIN (
    ...
    {
        // Bolt Optimization: Use a single INNER JOIN instead of nested subqueries.
        // This is much more efficient for calculating the total unread count (O(N+M) vs O(N*M)).
