...
class ChatMensaje
{
    ...
    {
        // Bolt Optimization: Replaced correlated scalar subqueries with JOINs to derived tables.
        // The unread count and latest message ID are now calculated in grouped derived tables
        // which include a participation filter to ensure O(N+M) complexity and avoid global table scans.
        $sql = "SELECT 
                    c.id,
                    c.tipo,
                    ...
                    COALESCE(unread.total, 0) AS no_leidos,
                ...
                LEFT JOIN (
                    ...
                    INNER JOIN chat_participantes cp_filter ON cm2.id_conversacion = cp_filter.id_conversacion
                ...
                -- Optimized last message retrieval
                LEFT JOIN (
                    SELECT m_inner.id_conversacion, MAX(m_inner.id) as last_id
                    FROM chat_mensajes m_inner
                    INNER JOIN chat_participantes cp_last ON m_inner.id_conversacion = cp_last.id_conversacion
                    WHERE cp_last.id_usuario = ?
                ...
                -- Info of the other participant (for direct chats)
    ...
    {
        // Bolt Optimization: Replaced nested subqueries with a single JOIN between
        // participants and messages. This ensures O(N+M) performance.
        $sql = "SELECT COUNT(*) AS total
                FROM chat_mensajes m
                INNER JOIN chat_participantes cp ON m.id_conversacion = cp.id_conversacion
                WHERE cp.id_usuario = ?
