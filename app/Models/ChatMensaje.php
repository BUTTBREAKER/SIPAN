...
class ChatMensaje
{
     ...
     * Bolt Optimization: Replaced scalar subqueries in SELECT and JOIN with derived tables (O(N+M)).
     * Derived tables are filtered by userId to maintain performance on large datasets.
     */
    public function getConversaciones($userId)
    {
        $sql = "SELECT 
                ...
                -- Bolt Optimization: Join with pre-calculated latest message IDs (O(N+M))
                LEFT JOIN (
                    SELECT id_conversacion, MAX(id) as last_id
                    FROM chat_mensajes
                    WHERE id_conversacion IN (SELECT id_conversacion FROM chat_participantes WHERE id_usuario = ?)
                ...
                -- Bolt Optimization: Join with pre-calculated unread counts (O(N+M))
                LEFT JOIN (
     ...
     * Bolt Optimization: Replaced correlated subquery with a single JOIN between
     * chat_mensajes and chat_participantes to achieve O(N+M) performance.
     */
