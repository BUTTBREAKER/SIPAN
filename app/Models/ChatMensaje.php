...
class ChatMensaje
{
    ...
    public function getConversaciones($userId)
    {
        $sql = "SELECT 
                ...
                -- Bolt: Reemplazo de subconsulta correlacionada por JOIN con tabla derivada para MAX(id) (O(N+M))
                LEFT JOIN (
                    SELECT m2.id_conversacion, MAX(m2.id) as last_id
                    FROM chat_mensajes m2
                    INNER JOIN chat_participantes cp_lm ON m2.id_conversacion = cp_lm.id_conversacion
                    WHERE cp_lm.id_usuario = ?
                    GROUP BY m2.id_conversacion
                ) lm ON lm.id_conversacion = c.id
                LEFT JOIN chat_mensajes m ON m.id = lm.last_id
                LEFT JOIN usuarios m_user ON m_user.id = m.id_usuario
                -- Bolt: Reemplazo de subconsulta correlacionada por JOIN con tabla derivada para conteo no leídos (O(N+M))
                LEFT JOIN (
    ...
    {
        // Bolt: Optimización de O(N*M) a O(M) usando un solo JOIN para el conteo global de no leídos
