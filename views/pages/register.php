...
<div
  ...
  <form @submit.prevent="submitForm" class="mt-5 xl:mt-7">
      ...
      <div>
        ...
        <div class="flex-1 w-full">
          <input
            ...
            name="name"
            x-model="name"
            :class="errors.name ? 'border-destructive' : 'border-default-300'"
        ...
        <template x-if="errors.name">
          <p class="text-sm text-destructive" x-text="errors.name"></p>
        </template>
      </div>
      <div>
        <label
    ...
    <label
      for="terms"
      class="inline-flex cursor-pointer items-center gap-2 text-sm font-medium text-default-600 [&:has(input:checked)]:text-default-900 [&:has(input:disabled)]:cursor-not-allowed [&:has(input:disabled)]:opacity-75 my-4">
    ...
    <button class="inline-flex items-center justify-center font-semibold ring-offset-background transition-colors focus-visible:outline-none focus-visible:ring-0 disabled:opacity-50 whitespace-nowrap disabled:pointer-events-none bg-primary text-primary-foreground hover:bg-primary/80 h-11 rounded-md px-[18px] py-[10px] text-base w-full">
      Create an Account
    </button>
  </form>

  <?php App::renderComponent('oauth-buttons') ?>
