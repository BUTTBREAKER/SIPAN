...
function testOptimizations()
{
    ...
    $proveedorModel->addInsumos(5, $insumosProveedor);
    ...
    foreach ($mockDb->queries as $q) {
        if (is_array($q) && strpos($q['sql'], 'INSERT INTO proveedor_insumos') !== false) {
            if (strpos($q['sql'], 'VALUES (?, ?, ?, ?), (?, ?, ?, ?)') !== false) {
                $batchInsertFound = true;
                echo "Found batch insert for proveedor insumos (multiple rows)\n";
                if ($q['params'] === [5, 10, 15.5, 3, 5, 11, 20.0, 5]) {
                    echo "✅ Parameters bound correctly for batch insert\n";
                } else {
                    echo "❌ Parameters bound incorrectly\n";
                }
            }
        }
    }
    ...
    if ($batchInsertFound) {
        echo "✅ Proveedor addInsumos optimization verified (Mocked)!\n";
    } else {
        echo "❌ Proveedor addInsumos optimization verification failed!\n";
    }
}
