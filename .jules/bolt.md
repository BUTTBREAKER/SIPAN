...
## 2025-08-15 - [Batching Multi-Row Updates via CASE Statements]
**Learning:** Performing database updates inside loops when deducting stock across multiple inventory lots (FIFO/FEFO) introduces an N+1 write bottleneck. Consolidating multi-row updates into a single `UPDATE` query using parameterized `CASE WHEN ... END` expressions reduces database executions from O(N) to O(1) while guaranteeing parameter safety.
**Action:** When updating multiple records based on sequential logic, accumulate the new states in-memory during iteration and execute a single consolidated `CASE WHEN` update query afterwards.
