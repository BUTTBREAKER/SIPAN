...
## 2025-01-24 - [Consolidated SQL Aggregation for Reports]
**Learning:** Calculating complex breakdowns (like payment methods) by looping through result sets in PHP is highly inefficient as data grows. Using SQL `UNION ALL` to bridge disparate table structures (e.g., modern `venta_pagos` and legacy `ventas` records) and aggregating directly in the database (`GROUP BY`, `SUM`) reduces application memory usage and CPU overhead.
**Action:** Shift aggregation logic from PHP loops to SQL queries wherever possible, especially for reports covering large date ranges.
