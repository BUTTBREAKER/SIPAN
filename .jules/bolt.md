...
## 2025-01-24 - [Batch INSERT for Provider Insumos]
**Learning:** Iterating over an array of items to execute single `INSERT` statements inside a loop (e.g. `addInsumos` in `Proveedor.php`) creates unnecessary database round-trips (O(N) queries). Refactoring to construct a single multi-row `INSERT INTO ... VALUES (?, ?, ?, ?), ...` statement converts the operation into O(1) database executions within the transaction.
**Action:** Always batch multi-row inserts into a single SQL statement when associating multiple items/relational entities to a parent model.
