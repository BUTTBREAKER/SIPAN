## 2025-01-25 - [Request-Level Cache Flag Regression]
**Learning:** Request-level cache flags (like `self::$tasaBcvChecked`) must be set to `true` whenever the cache is successfully populated (even from the database), not just from the external API fallback. Otherwise, subsequent lookups in the same request will bypass the static in-memory cache and keep hitting the database.
**Action:** Always set cache verification flags to true immediately upon populating static cache variables to fully optimize repeat execution paths.
