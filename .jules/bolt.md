...
## 2025-01-25 - [Request-Level Caching Verification Flag]
**Learning:** When implementing request-level caching in models (like `Configuracion::getTasaBCV`), failing to set the caching verification flag (`self::$tasaBcvChecked = true`) during database retrieval cache-miss bypasses subsequent cache lookups. This causes the application to hit the database on every lookup within the same request.
**Action:** Always verify that request-level cache hits are supported by setting the appropriate boolean flags during cache-miss retrieval, and validate caching logic via dedicated mocked tests.
