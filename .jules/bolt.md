## 2025-01-24 - [Request-Level BCV Exchange Rate Cache Fix]
**Learning:** Request-level cache mechanisms (like static class flags and in-memory key stores) are highly effective at preventing redundant database and API queries. However, if state variables are not updated when the state shifts (e.g. `self::$tasaBcvChecked` not being set to `true` during a fallback load of the exchange rate), the cache is rendered completely inactive on subsequent calls, causing performance regressions and redundant slow API calls.
**Action:** When implementing request-level caching, always ensure all state variables (e.g., loaded/checked boolean flags) are properly set on successful retrieval or fallback, and verified using test suites with assertions enabled (`-d zend.assertions=1`).
