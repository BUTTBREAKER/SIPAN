...
## 2025-01-24 - [Middleware-Driven Query Optimization]
**Learning:** High-frequency calls from middleware (like `AuthMiddleware::checkCaja()` calling `Caja::getActiva()`) are prime candidates for request-level caching. Without it, the same database query is executed on almost every single request, often multiple times if the controller also needs that data. Additionally, fixing routing typos (like `/cajas/aprir`) prevents unnecessary server overhead and improves application reliability.
**Action:** Implement static request-level caching for models frequently called by middleware or global layouts (e.g., Active Caja, Active Sucursal).
