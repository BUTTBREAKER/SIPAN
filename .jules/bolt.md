...
## 2025-01-24 - [Chat Polling and Correlated Subquery Optimization]
**Learning:** High-frequency polling endpoints (like chat sync every 15s) that use correlated subqueries for status counts (unreads) or latest message retrieval create a compounding load on the database. Replacing these with derived tables (joins on GROUP BY results) and utilizing primary keys (MAX(id)) instead of temporal ordering (ORDER BY created_at DESC LIMIT 1) reduces execution complexity from O(N*M) to O(N+M), where N is conversations and M is messages.
**Action:** Always optimize polling-related queries by flattening subqueries into joins and preferring PK-based lookups for "latest" record retrieval.
