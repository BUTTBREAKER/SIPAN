...
## 2025-01-24 - [SQL-Level Aggregation for Multi-Table Reports]
**Learning:** Manual aggregation in PHP over large datasets (e.g., summing payments by method) is a significant CPU and memory bottleneck. Offloading this to SQL using `UNION ALL` and `GROUP BY` allows the database to handle the heavy lifting and simplifies controller logic. This is especially useful when transitioning schemas, allowing for a unified view of legacy and modern data.
**Action:** Always prefer SQL-level aggregation (`SUM`, `GROUP BY`) for reporting dashboards and use `UNION ALL` to bridge discrepancies between old and new data structures during migrations.
