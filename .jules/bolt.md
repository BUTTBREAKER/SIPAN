...
## 2025-01-24 - [In-Memory Cache State Flags]
**Learning:** When using request-level static caching for database configurations (such as the BCV exchange rate), simply storing the retrieved value in the cache array is not always sufficient if another flag (like `self::$tasaBcvChecked`) is used to completely bypass the database query. Forgetting to set this flag on initial query will cause the method to repeatedly query the database, defeating the cache.
**Action:** Always ensure all static cache state flags/keys are properly set and updated together when caching a database result in-memory.
