...
## 2025-08-07 - [Configuracion Tasa BCV Cache Flag Assignment]
**Learning:** A performance regression in global configuration caching (`getTasaBCV()`) occurred because the static cache verification flag `self::$tasaBcvChecked` was not set to `true` after performing the first database fetch. Consequently, subsequent calls during the same request lifecycle bypassed the in-memory cache and executed redundant SQL queries.
**Action:** When implementing request-level caching with static flags, verify that the state/tracking flag is correctly updated on the initial cache population to prevent silent caching failures and redundant database lookups.
