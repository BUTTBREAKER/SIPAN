...
## 2025-01-25 - [Request-Level BCV Rate Cache Flag Reset Bug]
**Learning:** In multi-component single-request PHP life-cycles, a static class cache can still cause redundant database lookups if the internal state flag (such as `$tasaBcvChecked`) is not persistently set when the value is retrieved. Failing to set the flag resulted in multiple database queries per request for the global exchange rate, especially impacting high-frequency endpoints.
**Action:** Ensure that any request-level cache checker updates its internal flag status on both cache hit/miss and successful retrieval/fallback to avoid redundant checks.
